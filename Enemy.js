...
class Enemy {
    constructor() {
        ...
        this.type = random([1,2]) === 1 ? "bird" : "monster";//= type; // "bird" or "monster" // send in random here ex, 
    ...
    update() {
        ...
        if (collided){ //If collision == true then bob is dead
            bob.isDead = true;
            this.type === "bird" ? bob.reasonForDeath = "Bird" : bob.reasonForDeath = "Monster" 
        }
        // if passed give points
        else if (collided != true && bob.x > this.x + this.size && this.hasPassed != true) {
            bob.getPoints();
            this.hasPassed = true;
        }
    }
